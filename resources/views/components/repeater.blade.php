                ...
                @forelse ($fields as $f)
                    ...
                    @if ($f['type'] === 'file')
                        <div class="col-md-{{ $f['col'] ?? 6 }} col-12 ">
                            <x-repeater-file :required="$f['required'] ?? true" repeaterName="{{ $name }}" :multiple="$f['multiple'] ?? false"
                                name="{{ $f['name'] }}" />
                        </div>
                    @elseif ($f['type'] !== 'select')
                        <div class="col-md-{{ $f['col'] ?? 6 }} col-12 ">
                            <x-repeater-input required="{{ $f['required'] ?? 'true' }}" type="{{ $f['type'] ?? 'text' }}"
                                repeaterName="{{ $name }}" name="{{ $f['name'] }}" />
                        </div>
                    @endif
